...
#得到vgg_16 模型结果的 logits，end_points
...
#将16*16 放大2倍 ，先是用双线性插值生成卷积核，再反卷积
...
#将pool4的结果和放大两倍后的结果相加
upsampled_logits = upsampled_logits + aux_logits_16s

#获得vgg_16 pool3之后的结果
pool3_feature=end_points['vgg_16/pool3']

#将pool3之后的结果进行[1,1]卷积
with tf.variable_scope('vgg_16/fc8'):
    aux_logits_8s = slim.conv2d(pool3_feature, number_of_classes, [1, 1],
                                 activation_fn=None,
                                 weights_initializer=tf.zeros_initializer,
                                 scope='conv_pool3')
#将前面pool4相加的结果反卷积至跟pool3结果一样大
upsample_filter_np_x2_ = bilinear_upsample_weights(2,number_of_classes)

upsample_filter_tensor_x2_ = tf.Variable(upsample_filter_np_x2_, name='vgg_16/fc8/t_conv_x2_')

upsampled_logits_ = tf.nn.conv2d_transpose(upsampled_logits, upsample_filter_tensor_x2_,
                                          output_shape=tf.shape(aux_logits_8s),
                                          strides=[1, 2, 2, 1],
                                          padding='SAME')
#将pool3的结果和放大两倍后的结果相加
upsampled_logits_=upsampled_logits_+aux_logits_8s
                                          
#将相加后的结果进行8倍反卷积放大，其中upsample_factor表示放大的比例
upsample_filter_np_x8 = bilinear_upsample_weights(upsample_factor,
                                                   number_of_classes)

upsample_filter_tensor_x8 = tf.Variable(upsample_filter_np_x8, name='vgg_16/fc8/t_conv_x8')

upsampled_logits = tf.nn.conv2d_transpose(upsampled_logits_, upsample_filter_tensor_x8,
