...
def dict_to_tf_example(data, label):
    ...
    feature_dict = {
        ...
        'image/filename': dataset_util.bytes_feature(data[-15:].encode('utf8')),#将路径名最后15个字符截取出来作为文件名
...
def create_tf_record(output_filename, file_pars):
    # Your code here
    writer = tf.python_io.TFRecordWriter(output_filename)#生成TFRecord的写入流
    
    for img_path,lab_path in file_pars:
        tf_example=dict_to_tf_example(img_path,lab_path)
...
def read_images_names(root, train=True):
    ...
    label = []
    for fname in images:
        data.append('%s/JPEGImages/%s.jpg' % (root, fname))#原图
        label.append('%s/SegmentationClass/%s.png' % (root, fname))#分割的图片
    return zip(data, label)#返回一个（图片路径，标签路径）的元组
...
def main(_):
    ...
    train_files = read_images_names(FLAGS.data_dir, True)#得到一个（图片路径，标签路径）的元组
